...
for i in range(X.shape[0]):
    plt.scatter(
        X[i, 0], X[i, 1], marker=mapper[i], s=75, edgecolors="black", facecolors="none"
    )
...
for i in range(X.shape[0]):
    plt.scatter(
        X[i, 0], X[i, 1], marker=mapper[i], s=50, edgecolors="black", facecolors="none"
    )
...
for i in indices:
    plt.scatter(
        X[i, 0], X[i, 1], marker=mapper[y[i]], linewidth=3, s=100, facecolors="black"
    )
